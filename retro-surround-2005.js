...
let observer=null;
function isAuthVisible(){const shell=$('#authShell');return document.body.classList.contains('auth-locked')&&shell&&!shell.hidden}
...
body:not(.auth-locked) #retroSurround2005{display:none!important}
#retroSurround2005 .retro-side{position:fixed;top:28px;bottom:28px;width:min(470px,calc(50vw - 235px));display:flex;flex-direction:column;gap:14px;justify-content:center;overflow:auto;scrollbar-width:thin}
#retroSurround2005 .retro-side.left{left:24px}#retroSurround2005 .retro-side.right{right:24px}
...
@media(max-width:1220px){#retroSurround2005 .retro-side{width:min(300px,calc(50vw - 220px))}.retro-mini-grid{grid-template-columns:1fr}}
@media(max-width:1050px){#retroSurround2005{display:none!important}}
`;document.head.appendChild(st)}
function addPanels(){if($('#retroSurround2005'))return;const root=document.createElement('div');root.id='retroSurround2005';root.hidden=true;root.innerHTML=`
...
function mount(){if(!$('#authWindow'))return false;addStyle();addPanels();syncVisibility();if(!observer){observer=new MutationObserver(syncVisibility);observer.observe(document.body,{attributes:true,attributeFilter:['class']});const shell=$('#authShell');if(shell)observer.observe(shell,{attributes:true,attributeFilter:['hidden']})}return true}
